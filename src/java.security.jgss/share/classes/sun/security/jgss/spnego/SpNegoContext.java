...
public class SpNegoContext implements GSSContextSpi {
    ...
    public final byte[] initSecContext(InputStream is, int mechTokenSize)
        ...
        if (DEBUG) {
            System.err.println("Entered SpNego.initSecContext with " +
        ...
        try {
            if (state == STATE_NEW) {
                ...
                initToken = new NegTokenInit(DER_mechTypes, getContextFlags(),
                                        mechToken, null);
                if (DEBUG) {
                    System.err.println("SpNegoContext.initSecContext: " +
                ...
                if ((negoResult == SpNegoToken.NegoResult.ACCEPT_COMPLETE) ||
                    ...
                    if (isMechContextEstablished()) {
                        ...
                        retVal = mechToken;
                        if (DEBUG) {
                            System.err.println("SPNEGO Negotiated Mechanism = "
                                + internal_mech + " " +
            ...
            if (DEBUG) {
                if (retVal != null) {
                    System.err.println("SNegoContext.initSecContext: " +
    ...
    public final byte[] acceptSecContext(InputStream is, int mechTokenSize)
        ...
        if (DEBUG) {
            System.err.println("Entered SpNegoContext.acceptSecContext with " +
                               "state=" +  printState(state));
        }
        ...
        try {
            if (state == STATE_NEW) {
                ...
                if (DEBUG) {
                    System.err.println("SpNegoContext.acceptSecContext: " +
                                        "receiving token = " +
                ...
                // XXX Use logging API
                if (DEBUG) {
                    System.err.println("AcceptSecContext: state = " + state);
                }
            }
            if (DEBUG) {
                    System.err.println("SpNegoContext.acceptSecContext: " +
    ...
    /*private byte[] generateMechListMIC(byte[] mechTypes)
        ...
        if (mechTypes == null) {
            if (DEBUG) {
                System.err.println("SpNegoContext: no MIC token included");
            }
            return null;
        }
        ...
        try {
            ...
            if (DEBUG) {
                System.err.println("SpNegoContext: getMIC = " +
    ...
    private boolean verifyMechListMIC(byte[] mechTypes, byte[] token)
        ...
        if (token == null) {
            if (DEBUG) {
                System.err.println("SpNegoContext: no MIC token validation");
            }
            return true;
        }
        ...
        try {
            ...
            valid = false;
            if (DEBUG) {
                System.err.println("SpNegoContext: MIC validation failed! " +
                                        e.getMessage());
            }
        }
        return valid;
        ...
        for (int i = 0; i < supported_mechSet.length; i++) {
            for (int j = 0; j < mechSet.length; j++) {
                if (mechSet[j].equals(supported_mechSet[i])) {
                    if (DEBUG) {
                        System.err.println("SpNegoContext: " +
    ...
    public final boolean isMechContextEstablished() {
        if (mechContext != null) {
            ...
            if (DEBUG) {
                System.err.println("The underlying mechanism context has " +
