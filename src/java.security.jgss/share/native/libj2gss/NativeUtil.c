 ...
 *
 * The caller must call resetGSSBuffer() to free up the buffer's
 * resources.
 *
 * If the caller can still have access to the same jbytes at
 * resetGSSBuffer() time, then the caller should set wantCopy to false
 * to help avoid unnecessary copies, but then the caller must also
 * provide the same jbytes to resetGSSBuffer().  Else the caller should
 * set wantCopy to true and call resetGSSBuffer() with a NULL jbytes.
 */
